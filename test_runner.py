...
def connect_range(runner: Type[Address], tries: int) -> Type[Tester]:
    """
    ...
    while runner_port < runner_port + tries:
        ...
        # loop through 100 ports
            ...
            if err.errno == errno.EADDRINUSE:
                runner_port += 1
                continue
            raise socket.error(f"Could not bind to ports in range \
...
def serve() -> NoReturn:
    ...
    if not args.port:
        ...
        runner.port = int(args.port)
        server = Tester((runner.host, runner.port), TestHandler)
    print(server)
